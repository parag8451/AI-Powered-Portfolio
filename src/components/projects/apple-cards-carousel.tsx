    ...
    >
      <div className="relative w-full">
        ...
        <div className="mr-10 flex justify-end gap-2 md:mr-20">
          <button
            className="relative z-40 flex h-10 w-10 items-center justify-center rounded-full border border-neutral-200/50 bg-white/60 backdrop-blur-md shadow-sm transition-all duration-300 hover:bg-white/90 hover:shadow-md hover:shadow-purple-500/10 disabled:opacity-40 dark:border-neutral-700/50 dark:bg-neutral-800/60 dark:hover:bg-neutral-700/80"
            onClick={scrollLeft}
            disabled={!canScrollLeft}
          >
            <IconArrowNarrowLeft className="h-5 w-5 text-neutral-600 dark:text-neutral-300" />
          </button>
          <button
            className="relative z-40 flex h-10 w-10 items-center justify-center rounded-full border border-neutral-200/50 bg-white/60 backdrop-blur-md shadow-sm transition-all duration-300 hover:bg-white/90 hover:shadow-md hover:shadow-purple-500/10 disabled:opacity-40 dark:border-neutral-700/50 dark:bg-neutral-800/60 dark:hover:bg-neutral-700/80"
            onClick={scrollRight}
          ...
          >
            <IconArrowNarrowRight className="h-5 w-5 text-neutral-600 dark:text-neutral-300" />
          </button>
        </div>
      </div>
    </CarouselContext.Provider>
  ...
  return (
    <>
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        onClick={handleOpen}
        className="group relative z-10 flex h-80 w-56 flex-col items-start justify-start overflow-hidden rounded-3xl bg-gray-100 shadow-lg transition-shadow duration-300 hover:shadow-xl hover:shadow-purple-500/20 dark:bg-neutral-900"
        whileHover={{ y: -8, rotateY: 5, rotateX: -2 }}
        transition={{ type: 'spring', stiffness: 300, damping: 20 }}
      >
        <div className="absolute inset-x-0 top-0 z-30 h-full cursor-pointer bg-gradient-to-b from-black/70 via-transparent to-transparent transition-opacity duration-300 group-hover:from-black/50" />
